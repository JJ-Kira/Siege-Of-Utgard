...
namespace Game
{
    ...
    /// </summary>
    public class Grenade : UxrWeapon, IUxrPrecacheable
    {
        ...
        [SerializeField] private float _damageNear   = 50.0f;
        ...
        private void Explode()
        {
            ...
            if (_createPhysicsExplosion)
            {
                ...
                foreach (Collider targetCollider in colliders)
                {
                    ...
                    if (targetCollider.TryGetComponent<Entity>(out var entity))
                    {
                        // Calculate the distance to the enemy
                        float distance = Vector3.Distance(transform.position, entity.transform.position);
                        // Calculate proportional damage based on distance
                        float damagePercentage = Mathf.Clamp01(1 - (distance / _damageRadius));
                        int damageToDeal = Mathf.RoundToInt(_damageNear * damagePercentage);
                        Defender.Instance.DealDamage(damageToDeal, entity);
                    }
                }
            }
